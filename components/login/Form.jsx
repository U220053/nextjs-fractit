...
import insta from "../../public/assets/images/insta_white.svg";
...
const Form = () => {
  ...
  const [isInstaFollowed, setIsInstaFollowed] = useState(
    () => JSON.parse(localStorage.getItem("isInstaFollowed")) || false
  );
  ...
  const handleInstaFollow = () => {
    setIsInstaFollowed(true);
    localStorage.setItem("isInstaFollowed", true);
    window.open("https://instagram.com/FractItLabs", "_blank");
  };

  const handleClaim = async () => {
    if (!isTwitterFollowed || !isDiscordFollowed || !isInstaFollowed) {
      toast.error(
        "Please follow on  Twitter , Discord and Instagram to claim the faucet!"
      );
      return;
    }
    setLoading(true);
        ...
        >
          ...
          >
            {isInstaFollowed ? (
              ...
              >
                <span style={{ marginRight: "10px" }}>Followed on</span>
                <Images src={insta} alt="Discord" width={30} height={30} />{" "}
              </div>
            ) : (
              <div
                style={{
